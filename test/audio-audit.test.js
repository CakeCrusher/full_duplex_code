...
for (const track of ['input', 'output']) test(`API ${track} timestamps preserve and identify missing frames without claiming they were silent`, t => {
  ...
  audit.write(track,pcm,{startMs:200,endMs:400});
  audit.write(track,pcm,{startMs:600,endMs:800});
  const data=fs.readFileSync(path.join(dir,`${track}.wav`)).subarray(44);
  ...
  assert.equal(gap.reason,`missing_api_${track}`);
  assert.deepEqual([gap.startMs,gap.endMs,gap.durationMs],[400,600,200]);
  audit.write(track,pcm,{startMs:12200,endMs:12400});
  const afterPause=fs.readFileSync(path.join(dir,`${track}.wav`)).subarray(44);
