...
- **Walkthrough (default):** explain stages, design choices, problems and checks in detail. At completion, cover Claude's final answer: what was built, how to use it, validation, caveats and remaining work. Finish each thought, and skip superseded progress when work moves quickly.

All levels receive the same context. Your spoken requests take priority over the selected default: asking a question, changing topic, or asking Live to stop discussing something should lead the conversation. Claude’s observations are background reference material. Live decides whether an unsolicited update is useful; the bridge does not schedule speech cues or promote progress into commentary. The prompt asks Live to finish its current thought despite new observations, while yielding to you. Quiet also skips the opening greeting. These are model instructions, not a guarantee of exact spoken behavior. Recorded tests still show topic drift and interrupted explanations during busy sessions; the simpler prompt and source labels do not fully resolve that behavior.
...
The bridge keeps ordinary text fields and older observations. Only recognized binary attachment data is replaced for text context. A bounded prefix of saved history is supplied at session startup, before audio begins; remaining history and new events use quiet appends. It splits text into small appends for the Live API's per-append limit. Every thinking append carries a short source label, including continuation chunks. In Quiet or Milestones it also repeats the selected speaking mode, so a late fragment does not become a new request to narrate. The label does not remove or summarize the original content. The bridge sends thinking fragments in order without waiting for API acknowledgments. It sizes them using local token estimates with room below the API’s 500-token limit, including each fragment’s hook label. Only a backed-up network socket pauses writes; model acknowledgments are tracked separately. A large burst can still take time to enter the model after the bridge has sent it. Microphone and speaker activity never pause context delivery: hooks keep flowing while you or Live are talking. Operator priority is a conversational instruction, not a restriction on receiving Claude's data. The dashboard distinguishes fragments still waiting to be sent from fragments already sent and awaiting API acknowledgment. Those acknowledgments estimate context injection; they do not certify that Live understood every observation. Speaking-preference instructions are sent directly so they do not wait behind that queue. The bridge keeps the observations for a voice restart. An append failure ends voice with an error so reconnecting can replay the saved observations. A single local hook request has a 32 MiB transport limit; oversized events are reported instead of silently truncated. The model's own context capacity still applies.
