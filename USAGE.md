...
The bridge keeps ordinary text fields and older observations. Only recognized binary attachment data is replaced for text context. A bounded prefix of saved history is supplied at session startup, before audio begins; remaining history and new events use quiet appends. It splits text into small appends for the Live API's per-append limit. Every thinking append carries a short background-reference label, including continuation chunks, so quoted Claude text stays distinguishable from operator speech. The label adds context; it does not remove, summarize, or delay the original content. The bridge keeps the observations for a voice restart. An append failure ends voice with an error so reconnecting can replay the saved observations. A single local hook request has a 32 MiB transport limit; oversized events are reported instead of silently truncated. The model's own context capacity still applies.
